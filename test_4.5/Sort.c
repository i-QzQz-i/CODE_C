...
void QuickSortNonR(int* a, int left, int right)
{
	...
	while (!STEmpty(&s))
	{
		...
		int keyi = PartQSort1(a, begin, end);
...
void _MergeSortNonR(int* a, int* tmp, int n)
{
	...
	while (gap < n)
	{
		for (int j = 0; j < n; j += 2 * gap)
		{
			...
			memcpy(a + j, tmp + j, (i - j) * sizeof(int));
		}
